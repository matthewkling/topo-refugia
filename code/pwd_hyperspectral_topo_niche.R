...
cwd.even$counts
cwd.even
barplot(cwd.even$counts)
hypv$cwd.even.fac <- cut(hypv$cwd8110,breaks=cwd.even$breaks)

summary(hypv$southness)
south.even <- hist(hypv$southness,breaks=c(seq(-0.75,0.7,by=0.05)))
south.even$breaks
south.even$counts
barplot(south.even$counts)
hypv$south.even.fac <- cut(hypv$southness,breaks=south.even$breaks)
...
niche <- data.frame(matrix(NA,13,6))
names(niche) <- c('cwd.mean','cwd.occ.even','cwd.occ.perc','south.mean','south.occ.even','south.occ.perc')
